...
data = pd.read_excel('D:/Telugu_Node2vec/DATA.xlsx', header = None)
data.head()
    ...
    for i in data:
        ...
        for token in tokens:
            try:
                ...
                temp.append(np.zeros([1,50]))
                continue
        temp = np.array(temp)
        temp = np.mean(temp,axis=0)
        temp = np.reshape(temp, (1, 50))
...
#     loss = []
    ...
    for i in np.arange(1): 
        ...
        model.train()
        optimizer.zero_grad()
        output,latent = model(SC_data_u, torch.Tensor(PMI))
        #print('Output shape', str(latent.shape))
        train_features = generate_features(data[0].iloc[trainindex].values, latent.detach().numpy())
        val_features = generate_features(data[0].iloc[valindex].values, latent.detach().numpy())
    ...
    if not args.fastmode:
        ...
        for j in np.arange(1):
            ...
            np.save('sentiment_wordsPMI50',latent.detach().numpy())
            #print('Output shape', str(latent.shape))
        train_features = generate_features(data[0].iloc[trainindex].values, latent.detach().numpy())
        val_features = generate_features(data[0].iloc[valindex].values, latent.detach().numpy())
        print(train_features.shape, val_features.shape)
...
def test(model,testindex):
    ...
    for i in np.arange(1):
        ...
        #print('Output shape', str(latent.shape))
        train_features = generate_features(data[0].iloc[trainindex].values, latent.detach().numpy())
        test_features = generate_features(data[0].iloc[testindex].values, latent.detach().numpy())
